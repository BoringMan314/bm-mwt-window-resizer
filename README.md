...
Windows 上用來調整遊戲視窗外觀與尺寸的小工具。一般使用者請直接下載並執行倉庫根目錄的 [**MWT-1.0.exe**](MWT-1.0.exe)，不必另外尋找 Release 頁面。
...
1. **先開啟遊戲**，等到遊戲視窗出現。
2. **再開啟本工具**（雙擊 `MWT-1.0.exe`，或用 Python 執行 `mwt.py`）。
